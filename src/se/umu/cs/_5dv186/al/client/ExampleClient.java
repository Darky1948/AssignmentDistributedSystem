package se.umu.cs._5dv186.al.client;
...
public final class ExampleClient {
	...
	public static void listStreamInfo(StreamServiceClient client)
		...
		for (StreamInfo stream : streams) {
			System.out.println("  '" + stream.getName() + "': "
					+ stream.getLengthInFrames() + " frames, "
					+ stream.getWidthInBlocks() + " x "
					+ stream.getHeightInBlocks() + " blocks");
		}
	}
	...
	public static void main(String[] args) {
		try {
			...
			final int timeout = (args.length > 1) ? Integer.parseInt(args[1])
					: DEFAULT_TIMEOUT;
			final String username = (args.length > 2) ? args[2] : "test";

			StreamServiceClient client = DefaultStreamServiceClient.bind(host,
					timeout, username);
			...
			for (int i = 0; i < nr; i++) {
				try {
					...
					Block block = client
							.getBlock(stream, frame, blockX, blockY);
					long t2 = System.currentTimeMillis();
					System.out.println("block received from server in "
							+ (t2 - t1) + " ms");
					count++;
